...
export default function DashboardScreen() {
  ...
  const ensureProfile = useCallback(async (token: string | null): Promise<{ ok: boolean; error?: string }> => {
    try {
      ...
      const explorer = await AsyncStorage.getItem('dopamode_explorer');
      const risk = await AsyncStorage.getItem('dopamode_risk');
  ...
  }, [loadQuest, enrichQuestWithLocation]);
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <View style={styles.errorBox}>
          ...
          <Pressable style={styles.retryBtn} onPress={loadQuest}>
          ...
          <View style={[styles.questCard, (isAccepted || isCompleted) && styles.questCardAccepted]}>
            ...
            <View style={styles.questCardBody}>
              <View style={styles.missionBlock}>
                ...
                <Text style={styles.missionText}>{quest.mission}</Text>
...
const styles = StyleSheet.create({
  ...
  cardNameLabel: { fontSize: 10, fontWeight: '900', color: '#9a3412', letterSpacing: 1, marginBottom: 4 },
